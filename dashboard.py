...
def render_talent_page(employees, monthly):
    ...
    st.markdown("AI-powered resume analysis and candidate-job matching using a Routed NER ensemble (DistilBERT v6 + GLiNER + ModernBERT v11 + gazetteer, 10 entity types) and SBERT embeddings.")
    st.markdown("")
...
def render_boardroom_brief_page(employees, monthly, individual):
    ...
    st.markdown(
        ...
        "LLM-free orchestration over 23 tools. Every verdict traces to a "
        "named rule in the 20-cell decision matrix."
    )
    st.markdown("")
    ...
    with c2:
        if crewai_available:
            engine = st.selectbox(
                ...
                help=(
                    "Proprietary Brain: deterministic, ~500 ms per memo, "
...
def render_sidebar():
    with st.sidebar:
        ...
        st.markdown('''<div style="padding:8px 0;font-size:0.72rem;color:#86868B;line-height:1.8">
            ...
            Proprietary (default) &middot; CrewAI+Claude (optional)<br><br>
